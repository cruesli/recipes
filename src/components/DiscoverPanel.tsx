...
export function DiscoverPanel({ recipes, basePath }: Props) {
  ...
  return (
    <div style={{ width: '100%', minHeight: '60vh', display: 'flex', backgroundColor: 'var(--color-paper)' }}>
      ...
      >
        <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--color-ink-muted)', marginBottom: '4px' }}>
          Mood
        </p>
        <h2 style={{ fontSize: '1.5rem', color: 'var(--color-ink)', margin: '0 0 1.5rem' }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isActive ? '1px solid var(--color-oxblood)' : '1px solid transparent',
                  backgroundColor: isActive ? 'rgba(126,38,37,0.08)' : 'transparent',
                  color: isActive ? 'var(--color-oxblood)' : 'var(--color-ink)',
                  cursor: 'pointer',
                  fontSize: '13px',
        ...
        {expanded === 'mood' && activeMood && (
          <div style={{ marginTop: '1.5rem' }}>
            {moodMatches.length > 0 ? (
              <>
                <p style={{ fontSize: '12px', color: 'var(--color-ink-muted)', marginBottom: '10px' }}>
      ...
      >
        ...
        <h2 style={{ fontSize: '1.5rem', color: 'var(--color-ink)', margin: '0 0 1.25rem' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '44px', height: '44px', borderRadius: '50%', backgroundColor: 'var(--color-paper)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', flexShrink: 0 }}>
              🌍
            </div>
            <div>
              <p style={{ fontSize: '14px', fontWeight: 500, color: 'var(--color-ink)', margin: '0 0 2px' }}>{currentSpotlight}</p>
        ...
        <button
          ...
          style={{ fontSize: '12px', color: 'var(--color-oxblood)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline', alignSelf: 'flex-start', marginBottom: '1.5rem' }}
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {recipes.map((r) => (
              <a
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.backgroundColor = 'var(--color-stone)'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.backgroundColor = 'var(--color-surface)'; }}
              >
                {r.image && (
                  <img
                    src={`${basePath}${r.image}`}
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontSize: '13px', color: 'var(--color-ink)', margin: 0, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.title}</p>
                  <p style={{ fontSize: '11px', color: 'var(--color-ink-muted)', margin: 0, textTransform: 'capitalize' }}>{r.cuisine}</p>
                </div>
              </a>
            ))}
          </div>
